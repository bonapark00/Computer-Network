...
def accept_clients(the_server, y):
    while True:
        ...
        # 'clients' is the list of storing socket objs
...
def send_receive_client_message(client_connection, client_ip_addr):
    ...
    # sending end receiving msgs with multiple clients
    while True:
        data = client_connection.recv(4096).decode() # 0
        if not data: break
        if data == "exit": break
        ...
        if str(data) == "FILE":
            fileName = client_connection.recv(4096).decode() # 1
            lenOfFile = client_connection.recv(4096).decode() # 2
            for c in clients:
                if c != client_connection:
                    try:    # don't need...??
                        ...
                        time.sleep(0.1)
                        c.send(clients_names[idx].encode()) # check here..!
                        time.sleep(0.1)
                        print("\nThe File sender is "+clients_names[idx] +"\n\n")
                    except:
                        c.close()
            ...
            while str(total) != lenOfFile:
                data = client_connection.recv(4096) # 3
                ...
                for c in clients:
                    if c != client_connection:
                        try: 
                            ...
                            c.close()
                            print(str(i) + " th is closed")
                print("Sent to " + str(i))



            # broadcstFIle
        else : # just a message not a file
            client_msg = data
